"""
領域外の海流の影響を、既存の軌跡データだけで「軽量版」定量化する。
...
を調べる。外向き速度が大きい(=黒潮続流のように領域外へ強く運ばれている)なら、
「打ち切ってもどうせ返ってこない」ので現在の領域設定は妥当と言える。
逆に外向き速度が小さい/内向きに近い粒子が多いなら、格子際で往復している可能性があり、
領域を広げないと運命(漂着 or 漂流)を過小/過大評価している疑いが強まる、という論理。
