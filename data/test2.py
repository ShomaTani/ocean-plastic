...
# origin==0 の粒子群 → その放出源に対する「漂着分布」= 教師ヒートマップ1枚
sel = (origin == 0)
H, _, _ = np.histogram2d(flon[sel], flat[sel], bins=[gx, gy])
H = H.T
if H.sum() > 0:
    H = H / H.sum()
...
print(f"beached fraction     : {beached.mean():.2f}   (0や1に張り付いてたら要注意)")
print(f"mean displacement    : {np.nanmean(disp):.3f} deg  (>0 でないと移流できてない)")
print(f"max  displacement    : {np.nanmax(disp):.3f} deg")
print(f"density map sum(org0) : {H.sum():.3f}   (1.0 なら正規化OK)")
print(f"density nonzero cells : {(H > 0).sum()}")
...
try:
    ...
    plt.pcolormesh(gx[:-1], gy[:-1], H, shading="auto")
    plt.scatter(RELEASE_SITES[0][0], RELEASE_SITES[0][1], c="r", marker="*", s=120,
                label="origin 0")
    plt.title("beaching density from origin 0"); plt.legend()
    plt.colorbar(label="prob."); plt.tight_layout()
    plt.savefig("beaching_density_origin0.png", dpi=110)
    print("saved beaching_density_origin0.png")
